...
allocators = ['LRU2Q']
wakeUpRebalancerEveryXReqs = [i * 1000 for i in [100]]
...
rebalanceStrategies = {
    "marginal-hits": [
        ...
        for wakeup in wakeUpRebalancerEveryXReqs + [1000, 10_000, 500_000, 1000_000]
    ], 
    ...
    "free-mem": [{"rebalanceDiffRatio": 0.25, "wakeUpRebalancerEveryXReqs": wakeup} for wakeup in wakeUpRebalancerEveryXReqs],
    ...
    for allocator, (rebalanceStrategy, rebalanceParamsList) in \
        ...
        for rebalanceParams in rebalanceParamsList:  # Iterate over list of configurations
            for cache_size in info['cache_sizes']:
                if 'strategies' in info and rebalanceStrategy not in info['strategies']:
                    continue
...
def run_cachebench_and_extract_info(config_file, output_json_file, stat_json_file):
    command = (
        ...
        f"--dump_result_json_file {stat_json_file}"
    )
...
def run_experiment(index, config):
    subdir = 'outcome_detail/' + config['memo_config']['uuid']
    ...
    stat_json_file = os.path.join(subdir, 'stats.json')
    
    return_code = run_cachebench_and_extract_info(cachebench_config_file_path, output_json_file, stat_json_file)
    if return_code == 0:
