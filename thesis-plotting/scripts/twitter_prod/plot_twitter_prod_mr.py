...
def create_twitter_prod_boxplots(csv_file):
    ...
    df = df[(df['trace_name'].str.startswith('twitter')) & (df['tag'] != 'warm-cold') & 
            (df['rebalance_strategy'] != 'lama') & df['miss_ratio_reduction_from_lru_disabled'].notna()]
    ...
    for wsr in wsr_values:
        ...
        pos = 0
        # Define strategy order (same for all allocators since LAMA is filtered out)
        ...
        legend_elements = []
        # Use consistent strategy order for legend (without LAMA)
        legend_strategy_order = ["disabled", "tail-age", "eviction-rate", "hits", "marginal-hits", "marginal-hits-tuned"]
        ...
        # Save to PDF
        output_file = f'twitter_prod_boxplot_wsr_{wsr:.2f}.pdf'
