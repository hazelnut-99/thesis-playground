...
def create_twitter_prod_worst_case_mr_barplots(csv_file):
    ...
    df = df[(df['trace_name'].str.startswith('twitter')) & (df['tag'] != 'warm-cold') & df['miss_ratio_reduction_from_lru_disabled'].notna()]
    ...
    for wsr in wsr_values:
        ...
        # Save to PDF
        output_file = f'twitter_prod_worst_case_mr_reduction_wsr_{wsr:.2f}.pdf'
