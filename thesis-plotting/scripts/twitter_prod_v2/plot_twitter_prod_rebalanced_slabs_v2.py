...
def create_twitter_prod_rebalanced_slabs_boxplots(csv_file):
    """
    Create box plots for Twitter production data showing number of rebalanced slabs.
    """
    
    # Read the CSV file
    df = pd.read_csv(csv_file)
    df = df[(df['trace_name'].str.startswith('twitter')) & (df['tag'] != 'warm-cold') & 
            df['n_rebalanced_slabs'].notna() & (df['rebalance_strategy'] != 'disabled')]
    
    # Define strategy labels and colors (excluding disabled since it never triggers rebalancing)
    strategy_labels = {
    ...
    for wsr in wsr_values:
        ...
        for i, allocator in enumerate(allocator_order):
            ...
            for strategy in current_strategy_order:
                ...
                if not strategy_data.empty:
                    values = strategy_data['n_rebalanced_slabs'].values
                    print(f"  {strategy} values: min={values.min():.0f}, max={values.max():.0f}, mean={values.mean():.0f}")
        ...
        # Save to PDF
        output_file = f'twitter_prod_rebalanced_slabs_wsr_{wsr:.2f}.pdf'
...
if __name__ == "__main__":
    create_twitter_prod_rebalanced_slabs_boxplots("../data/end-to-end/report_complete_processed.csv")
