...
def create_meta_kv_plot(csv_file, version="complete", trace_name="meta_202210_kv"):
    """
    ...
    # Define strategy labels and colors (including marginal-hits-tuned for slide 4)
    if version == "with_tuned":
        strategy_labels = {
        ...
        strategy_colors = {
            ...
            "marginal-hits-tuned": "#2E8B57"  # Sea green
        }
    else:
        strategy_labels = {
            "disabled": r"$\mathit{Disabled}$",
    ...
    if version == "disabled_only":
        strategy_order = ["disabled"]
    elif version == "with_tuned":
        strategy_order = ["disabled", "tail-age", "eviction-rate", "hits", "marginal-hits", "marginal-hits-tuned"]
    else:
    ...
    if version == "disabled_only":
        output_file = f'meta_kv_{trace_suffix}_slide1.pdf'
    elif version == "complete":
        output_file = f'meta_kv_{trace_suffix}_slide2.pdf'
    elif version == "with_tuned":
        output_file = f'meta_kv_{trace_suffix}_slide4.pdf'
    else:  # full_complete
        output_file = f'meta_kv_{trace_suffix}_slide3.pdf'
...
def create_rebalanced_slabs_plot(csv_file, trace_name="meta_202210_kv"):
    """
    ...
    # Filter for specified trace and remove LAMA and marginal-hits-tuned
    df = df[(df['trace_name'] == trace_name) & 
            (df['rebalance_strategy'] != 'lama') &
            (df['rebalance_strategy'] != 'marginal-hits-tuned')]
    ...
    # Define strategy labels and colors (same as original)
    strategy_labels = {
        "disabled": r"$\mathit{Disabled}$",
        "tail-age": r"$\mathit{Tail\text{-}Age}$",
        "eviction-rate": r"$\mathit{Eviction\text{-}Rate}$",
    ...
    # Set up matplotlib for publication quality (same settings)
    plt.rcParams.update({
        'font.size': 26,           
        'axes.titlesize': 30,      
        'axes.labelsize': 28,      
    ...
    # Save to PDF
    trace_suffix = trace_name.replace('meta_', '').replace('_kv', '')
    output_file = f'meta_kv_{trace_suffix}_rebalanced_slabs.pdf'
    plt.savefig(output_file, format='pdf', dpi=300, bbox_inches='tight',
               facecolor='white', edgecolor='none')
...
if __name__ == "__main__":
    ...
    for trace_name in trace_names:
        ...
        df_analysis = pd.read_csv("../data/end-to-end/report_complete_processed.csv")
        ...
        create_meta_kv_plot("../data/end-to-end/report_complete_processed.csv", version="disabled_only", trace_name=trace_name)
        
        print(f"Generating LRU-focused version (slide 2) for {trace_name}...")
        create_meta_kv_plot("../data/end-to-end/report_complete_processed.csv", version="complete", trace_name=trace_name)
        
        print(f"Generating full complete version (slide 3) for {trace_name}...")
        create_meta_kv_plot("../data/end-to-end/report_complete_processed.csv", version="full_complete", trace_name=trace_name)
        
        print(f"Generating version with marginal-hits-tuned (slide 4) for {trace_name}...")
        create_meta_kv_plot("../data/end-to-end/report_complete_processed.csv", version="with_tuned", trace_name=trace_name)
        
        print(f"Generating rebalanced slabs plot for {trace_name}...")
        create_rebalanced_slabs_plot("../data/end-to-end/report_complete_processed.csv", trace_name=trace_name)