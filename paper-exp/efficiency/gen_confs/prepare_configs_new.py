...
trace_names = [f'twitter_cluster{i}' for i in range(1, 55) if i != 25]
...
def generate_configs(
    ...
    work_dir=f"{HOME_DIR}/thesis-playground/paper-exp/efficiency/work_dir_new",
    force_delete=False
):
    total_confs = 0
    new_confs = 0
    ...
    if not force_delete and obsolete_count > 0:
        print(f"Found {obsolete_count} obsolete config directories (use force_delete=True to remove them)")
