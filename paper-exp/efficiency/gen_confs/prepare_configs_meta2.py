...
trace_names = ['meta_202206_kv_traces_all', 'meta_202401_kv_traces_all_sort'] 
working_set_ratios = [0.001, 0.005, 0.01, 0.02, 0.05, 0.1]
...
cache_configs = {
    ...
    "marginal-hits-new": [
        ...
        for aiad_step, wakeup in itertools.product([1, 2], rebalance_intervals)
    ] + [
        {
            "wakeUpRebalancerEveryXReqs": wakeup,
            "mhMovingAverageParam": 0.3,
        ...
        for fixed_thresh, wakeup in itertools.product([5, 10, 20, 50, 100], rebalance_intervals)
    ],
...
def generate_configs(
    ...
    work_dir=f"{HOME_DIR}/thesis-playground/paper-exp/efficiency/work_dir_meta_new",
    force_delete=False
):
    total_confs = 0
    new_confs = 0
    ...
    for allocator_config, (trace_name, trace_info), slab_size in itertools.product(allocators, trace_info_dict.items(), [1, 4]):
