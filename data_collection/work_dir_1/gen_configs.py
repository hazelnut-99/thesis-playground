...
cache_configs = {
    "rebalanceMinSlabs": [(1, True), (2, False), (3, False), (4, False)],
    "mhMovingAverageParam": [(0.1, False), (0.2, False), (0.3, True), (0.4, False), (0.5, False)],
    "mhMaxFreeMemSlabs": [(1, True), (2, False), (3, False)],
}
...
for trace_name, info in trace_info_dict.items():
    ...
    for config, allocator, ws_ratio in itertools.product(
        ...
        cache_config = {
            ...
            "rebalanceMinSlabs": config["rebalanceMinSlabs"][0],
            "mhMovingAverageParam": config["mhMovingAverageParam"][0],
            "mhMaxFreeMemSlabs": config["mhMaxFreeMemSlabs"][0]
        }
        test_config = {
            "numOps": op_count,
            "traceFileNames": [path]
